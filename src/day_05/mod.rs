...
fn solve_part_01(rules: HashMap<String, HashMap<String, usize>>, order_map: HashMap<i32, Vec<i32>>, updates: Vec<String>) {
...
fn read_file() -> (HashMap<String, HashMap<String, usize>>, HashMap<i32, Vec<i32>>, Vec<String>) {
    ...
    for constraint in parts[0].lines() {
        let parts: Vec<i32> = constraint.split('|')
            .map(|s| s.parse::<i32>().unwrap())
            .collect();
        
        // Ensure the order is represented (parts[0] must be before parts[1])
        order_map.entry(parts[0]).or_insert(Vec::new()).push(parts[1]);
    }
